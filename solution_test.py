...
from solution import Ingredient, RecipeOptimizer

TEST_CSV = """Raw Material ID,Similarity Index,Melting Point,Availability in Country,Price
...
@pytest.fixture
def optimizer():
    return RecipeOptimizer(StringIO(CSV_CONTENT))
...
def test_ingredient_availability():
    ...
    assert not ing.is_available_in_china()

def test_find_substitute(optimizer):
    sub = optimizer.find_substitute('6Z9K9FXGBN9Y1GXA')
